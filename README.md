...
| `RESEND_API_KEY` | **Render plan gratuito:** obligatoria para enviar correo; usa HTTPS (Resend). Si está definida, agenda no usa SMTP para el envío. |
| `SLOTONE_MAIL_REPLY_TO` | Opcional. Ej. `hola.slotone@gmail.com` — cabecera **Responder a** (útil si `SLOTONE_MAIL_FROM` es una dirección de Resend, no Gmail). |
| `SMTP_HOST` | Solo envío SMTP (local o host que permita puerto 587). En Render free no llegará a Gmail. |
...
- **Solo plan gratuito en Render:** en **`agenda`** definí `RESEND_API_KEY`, `SLOTONE_MAIL_FROM` (dirección permitida por Resend) y opcionalmente `SLOTONE_MAIL_REPLY_TO=hola.slotone@gmail.com` para que “Responder” vaya a esa cuenta. **No** intentes Gmail SMTP (`smtp.gmail.com:587`) en Render free: está bloqueado.
- **Solo en tu máquina:** podés usar Gmail SMTP como en `backend/agenda/mail-env.example` (bloque `desarrollo`).
